...
router.post('/api/users', async (req, res) => {
  ...
  try {
    ...
    const userPackage = await newUser.userWelcomePackage()
...
router.post('/api/users/user/verify', auth, async (req, res) => {
...
router.patch('/api/users/user', auth, async (req, res) => {
  ...
  const allowedUpdate = ['name', 'password', 'noteName', 'noteSections', 'noteTags', 'biography']
...
router.delete('/api/users/user', auth, async (req, res) => {
  ...
  try {
    ...
    await Section.deleteMany({ owner: user._id })
