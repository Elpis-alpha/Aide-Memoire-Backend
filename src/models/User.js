...
const { welcomeNote } = require('../mail/noteTypes');
...
// send verification mail
userSchemer.methods.userWelcomePackage = async function () {

  const user = this

  try {

    const favSec = new Section({

      owner: user._id, name: "Favorite", canDelete: false,

      description: `A special section for keeping special (favored) notes.`

    })

    const impSec = new Section({

      owner: user._id, name: "Important", canDelete: false,

      description: `A special section for keeping notes of great significance or value.`

    })

    const welcome = new Note({

      name: "Welcome", canDelete: false,

      isPublic: false, specialName: "welcome",

      text: welcomeNote(), owner: user._id,
